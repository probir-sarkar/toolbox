...
export default function Thumbnail(props: ThumbnailProps) {
  ...
  const isArchive =
    ...
    ["zip", "x-7z-compressed", "x-zip-compressed", "x-rar-compressed", "x-tar", "x-gzip", "x-apple-diskimage"].includes(
      fileTypeSpecific
    );
  ...
  return (
    <div
      ...
      className="relative overflow-hidden bg-gray-100 rounded-lg flex items-center justify-center"
      style={{
        width,
        height,
        aspectRatio: "1"
    ...
    >
      {showThumbnail ? (
        <Image fill className="w-full h-full object-cover" src={objectUrl} alt={props.file.name} />
      ) : null}
      {!showThumbnail ? (
        <div className="flex flex-col items-center justify-center w-full h-full">
          <div className="flex-1 flex items-center justify-center w-full">
            {!props.file.type ? (
              <svg aria-hidden="true" className="w-3/4 h-3/4" viewBox="0 0 25 25">
